...
inline __device__ float3 map(float3 coordinates, float *d_projection_matrices,
  ...
  return make_float3(matrix[0] * coordinates.x + matrix[1] * coordinates.y +
                         matrix[2] * coordinates.z + matrix[3],
                     matrix[4] * coordinates.x + matrix[5] * coordinates.y +
                         matrix[6] * coordinates.z + matrix[7],
                     matrix[8] * coordinates.x + matrix[9] * coordinates.y +
                         matrix[10] * coordinates.z + matrix[11]);
}
...
__global__ void backproject_3Dcone_beam_kernel_tex_interp(
  ...
  const float3 coordinates =
      index_to_physical(make_float3(i, j, k), volume_origin, volume_spacing);
  ...
  for (int n = 0; n < number_of_projections; ++n) {
    ...
    ip.x *= ip.z;
    ip.y *= ip.z;
    val += tex2DLayered(sinogram_as_texture, ip.x + 0.5, ip.y + 0.5, n) * ip.z *
           ip.z;
  }
 ...
 * cudaMalloc for the projection_matrices array :
 * https://stackoverflow.com/questions/48580580/tensorflow-new-op-cuda-kernel-memory-managment
 *
 */
void Cone_Backprojection3D_Kernel_Tex_Interp_Launcher(
    const float *sinogram_ptr, float *out, const float *projection_matrices,
    ...
    const float volume_spacing_z, const float volume_origin_x,
    const float volume_origin_y, const float volume_origin_z,
  ...
  float3 volume_origin =
      make_float3(volume_origin_x, volume_origin_y, volume_origin_z);
  ...
  cudaExtent projExtent =
      make_cudaExtent(detector_size.x, detector_size.y, number_of_projections);

  cudaArray *projArray;

  static cudaChannelFormatDesc channelDesc = cudaCreateChannelDesc<float>();
  gpuErrchk(cudaMalloc3DArray(&projArray, &channelDesc, projExtent,
                              cudaArrayLayered));

  auto pitch_ptr = make_cudaPitchedPtr(const_cast<float *>(sinogram_ptr),
                                       detector_size.x * sizeof(float),
                                       detector_size.x, detector_size.y);
  ...
  gpuErrchk(
      cudaBindTextureToArray(sinogram_as_texture, projArray, channelDesc));
