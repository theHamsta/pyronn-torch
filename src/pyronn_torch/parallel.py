...
class _ForwardProjection(torch.autograd.Function):
    ...
    def forward(self, volume, state = None):
        ...
        projection = torch.zeros(state._projection_shape, requires_grad=True).cuda()
    ...
    def backward(self, projection_grad, state = None, *args):
        ...
        volume_grad = torch.zeros(state._volume_shape, requires_grad=True).cuda()
...
class ParallelProjector:
    ...
    def __init__(self, detector_origin = -128, detector_spacing = 1, angles = torch.linspace(0, 360, 360), 
                volume_origin = [-128, -128], volume_shape = [256, 256], volume_spacing = [1, 1]):
        self._volume_shape = volume_shape
        self._volume_origin = volume_origin
        self._volume_spacing = volume_spacing
    ...
    def _calc_ray_vectors(self, angles):
        ...
        for i in range(angles.shape[0]):
            self._ray_vectors[i, 0] = torch.cos(angles[i])
            self._ray_vectors[i, 1] = torch.sin(angles[i])
    ...
    def forward_project(self, volume):
        ...
        projs = torch.zeros(volume.shape[0], self._projection_shape[0],
            self._projection_shape[1]).cuda()
